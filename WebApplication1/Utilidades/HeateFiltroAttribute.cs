...
{
    public class HeateFiltroAttribute:ResultFilterAttribute
    {
        protected bool debeIncluir(ResultExecutingContext context)
        {
            ...
            var cabezera = context.HttpContext.Request.Headers["incluir"];
            if (cabezera.Count == 0)
            {
                return false;
            }

            var valor = cabezera[0];
